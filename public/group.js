...
$('#dialogUnderlay,div.dialog,main').hide();
...
$(function() {
  ...
  /*
  socket.on("sendstatus", () => {
    if (door) {
      console.log("request to send status");
      let a = [];
      for (let i = 0; i < systems.length; i++) {
        a.push(stringify($("#system"+i)));
      }
      console.log(a);
      socket.emit("svg", {str: a, systems: systems});
    }
  ...
  */
  ...
  socket.on("open", o => {
    ...
    if (o.str) {
      for (let i = 0; i < o.str.length; i++) {
        let html = $.parseXML(o.str[i]);
        let children = $(html).children().detach();
        $("#system"+i).html(children);
      }
    }
    ...
    svg = document.getElementsByClassName('system');
    $("ul#entrants").html('<li>'+entrants.map(e => e.name).join("</li><li>")+'</li>');
    
    o.happenings.forEach(happening);
    $("#enter").hide();
    $("main").show();
  });
  ...
  socket.on("entrance", o => {
    if (door) {
      console.log("entrance");
      entrants.push(o);
      $("ul#entrants").append("<li>"+o.name+"</li>");
    }
  ...
  socket.on("exit", n => {
    if (door) {
      let i = entrants.findIndex(e => e.name === n);
      if (i > -1) {
        entrants.splice(i,1);
        $("#entrants li:nth-child("+(i+1)+")").remove();
      }
    }
  });
  
  
  socket.on("happening", happening);
  
  
  // enter button click || hit enter in secret input field
  ...
  $("#numstaves").on("change", numstaves);
  
  //change system width (applies to every system)
  $("#syswidth").on("change", changewidth);
  
  
  $("#shortenstaffOK").on("click", shortenStaff);
  
  $("#removestaffOK").on("click", removeStaff);
  
  $("#removesystemOK").on("click", removeSys);
      ...
      if (["stemup", "stemdown", "flagup", "flagdown", "beamabove", "beambelow", "tieabove", "tiebelow", "increasebeam"].includes(label) && notestems.length) {
        ...
        switch (label) {
          case "stemup": case "stemdown": case "flagup": case "flagdown":
            o.numbeams = numbeams;
            o.event = "stems";
            stemflag(notestems,label,current,numbeams);
            break;
          case "beamabove": case "beambelow":
            o.numbeams = numbeams;
            o.event = "stems";
            addbeam(notestems,label,current,numbeams);
            break;
  ...
  function enter(e) {
    ...
    let duplicate = namelist.includes(name);
    if (duplicate) {
      $("#name").val("");
      $("#name").attr("placeholder", '"'+name+'" already in use; pick another name');
    } else if (name.length && !/^\s+$/.test(name) && secret.length && !/^[^\w]+$/.test(secret) && !badchar) {
  ...
  function happening(obj) {
    
    if (door) {
      //console.log("happening received");
      //console.log(obj);
      switch (obj.event) {
        case "change":
          $("#message").text(obj.name + " changed something");
          $(obj.id).val(obj.val);
          if (obj.remove) {
            switch (obj.id) {
              case "#numsystems":
                ...
                break;
              case "#numstaves":
                removeStaff(null, obj);
                break;
              case "#syswidth":
                shortenStaff(null, obj);
                break;
            }
          } else {
            switch (obj.id) {
              ...
              case "#numstaves":
                ...
                break;
              case "#syswidth":
                changewidth(null, obj);
                break;
            }
          }
          break;
        case "erase":
          $("#message").text(obj.name + " erased something");
          erase(obj);
          break;
        case "add":
          $("#message").text(obj.name + " added something");
          let result = addthings(obj);
          finishdraw(result);
          break;
        case "stems":
          
          if (["stemup", "stemdown", "flagup", "flagdown"].includes(obj.label)) {
            $("#message").text(obj.name + " added stems");
            stemflag(obj.notestems, obj.label, obj.system, obj.numbeams);
          } else {
            $("#message").text(obj.name + " added a beam");
            addbeam(obj.notestems, obj.label, obj.system, obj.numbeams);
          }
          break;
        case "ties":
          $("#message").text(obj.name + " added ties");
          addties(obj.notestems, obj.label, obj.system);
          break;
        case "beam":
          $("#message").text(obj.name + " added a secondary beam");
          increasebeam(obj.notestems,obj.beam);
          break;
      }
  ...
  function numstaves(e, evt) {
    ...
    if (num > systems[0].staves.length) {
      ...
      if (!evt) {
        let o = {event: "change", id: "#numstaves", val: num};
        historyy.push(o);
        o.name = name;
        socket.emit("happening", o);
      }
  ...
  function numsystems(e, evt) {
    ...
    if (num > systems.length) {
      ...
      if (!evt) {
        let o = {event: "change", id: "#numsystems", val: num};
        historyy.push(o);
        o.name = name;
        socket.emit("happening", o);
      }
  ...
  function changewidth(e, evt) {
    ...
    if (width > syswidth) {
      ...
      syswidth = width;
      if (!evt) {
        let o = {event: "change", id: "#syswidth", val: width};
        historyy.push(o);
        o.name = name;
        socket.emit("happening", o);
      }
  ...
  function removeSys(e, evt) {
    ...
    cancelDialog();
    if (!evt) {
      let o = {event: "change", id: "#numsystems", val: num, remove: true};
      historyy.push(o);
      o.name = name;
      socket.emit("happening", o);
    }
  ...
  function removeStaff(e, evt) {
    ...
    removethings(2,num);
    if (!evt) {
      let o = {event: "change", id: "#numstaves", val: num, remove: true};
      historyy.push(o);
      o.name = name;
      socket.emit("happening", o);
    }
  ...
  function shortenStaff(e, evt) {
    let width = evt ? evt.val : reset;
    $(".stafflines rect").attr("width", width);
    $("svg.system").attr("width", width+50);
    systems.forEach(sys => sys.width = width);
    syswidth = width;
    ...
    if (!evt) {
      let o = {event: "change", id: "#syswidth", val: width, remove: true};
      historyy.push(o);
      o.name = name;
      socket.emit("happening", o);
    }
  ...
  function handleInput(e) {
    ...
    if (label) {
      switch (label) {
        ...
        case "eraser":
          output.event = "erase";
          output.target = copyTarget(e.target); //this needs to actually be a copy
          historyy.push(output);
          output.name = name;
          //console.log(output);
          socket.emit("happening", output);
          erase(output);
          break;
        default:
          switch (e.type) {
          ...
          if (output.x) {
            ...
            output.event = ["touchmove"].includes(e.type) ? "move" : "add";
            output.system = current;
            output.stave = stave;
            output.type = e.type;
            output.label = label;
            output.noteheadlock = noteheadlock;
            output.target = copyTarget(e.target); //probably need to copy this?

            historyy.push(output);
            if (["touchend","touchcancel","click"].includes(e.type)) {
              let o = {name: name};
              for (let key in output) {
                o[key] = output[key];
              }
              o.type = "click";
              //o.currenttouch = currenttouch;
            ...
            if (e.type === "touchmove") {
              if (currenttouch) {
                result = handleMove(output, currenttouch);
              ...
              if (currenttouch) {
                ...
                if (result.id.startsWith("note")) {
                  ...
                  if (lock.length && output.noteheadlock) {
                    locknotes(lock, current, stave, o);
...
function stemflag(notestems, label, current, num) {
...
function addbeam(notestems, label, current, num) {
  ...
  if (!$("#"+id).length) {
    ...
    for (let i = 0; i < notestems.length; i++) {
      ...
      staff.path($('g[id="'+parent+'"]'), `M ${stemx} ${y} v ${v}`, {id: stemid, fill: "none", "stroke-width": "1.5"});
...
function addthings(o) {
  let result = {stave: o.stave, system: o.system};
  let parent;
  ...
  if (o.label.startsWith("note")) {
    ...
    if (lock.length && o.noteheadlock && o.type === "click") {
      locknotes(lock, o.system, o.stave, o)
    }
    ...
    if (o.target.parent.classes.includes("note") || o.target.parent.classes.includes("rest")) {
      let arr = o.target.parent.id.split("-");
      o.x = Number(arr[3]);
      o.y = Number(arr[4]);
  ...
  if (["ledger","sharp", "natural", "flat"].includes(o.label)) {
    parent = $(parentstr);
...
function locknotes(lock, current, stave, o) {
...
function erase(e) {
  if (e.target.parent.classes.includes("clef") || e.target.parent.classes.includes("note") || e.target.parent.classes.includes("rest") || e.target.parent.classes.includes("timesig") ) {
    let thing = $('g[id="'+e.target.parent.id+'"]');
    let arr = e.target.parent.id.split("-");
  ...
  } else if (e.target.parent.classes.includes("barline") || e.target.parent.classes.includes("timesig")) {
    //also need to remove barlines from the collection
    $("#"+e.target.parent.id).remove();
  } else if (["path", "text", "circle"].includes(e.target.tagName) && e.target.id) {
    $(e.target.tagName+'[id="'+e.target.id+'"]').remove();
  } 
}
...
function finishdraw(r) {
  if (r.id && (r.id.startsWith("note") || r.id.includes("rest"))) {
    systems[r.system].staves[r.stave].notes.push(r.obj);
  } else if (r.obj && ["treble", "bass", "ledger", "cclef"].includes(r.obj.label)) {
    systems[r.system].staves[r.stave].things.push(r.obj);
  }
}
...
function handleMove(o, ct) {
  ...
  let y = calcy(o);
  ct.info.y = y;
  if (ct.info.staffpos || ct.info.staffpos === 0) {
    ct.info.staffpos = staffpos;
  }
  result.id = movething(ct.elementid,null,y);
  //if (o.label === "ledger") alert(ct.elementid + "\n" + result.id);
  result.obj = ct.info;
  return result;
}
...
function copyTarget(target) {
  let o = {
    ...
    parent: {
      classes: $(target).parent().attr("class").split(" "),
...
function removethings(idx, num) {
  ...
  $(".clef,.notes > path,.stems > path,.timesigs > path").each(function(i) {
